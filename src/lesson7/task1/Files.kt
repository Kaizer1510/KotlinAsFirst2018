...
fun alignFileByWidth(inputName: String, outputName: String) {
    val lineList = File(inputName).readLines().map {
        Regex("""(?<!\S)\s+|\s+(?!.)""").replace(it, "")
    }.toMutableList()
    if (lineList.size > 1) {
        val lengthList = lineList.map { it.length }
        val maxLength = lengthList.max()!!
        for (i in 0 until lineList.size) {
            val extraSpaces = maxLength - lengthList[i]
...
fun top20Words(inputName: String): Map<String, Int> {
    ...
    File(inputName).forEachLine { line ->
        line.toLowerCase().split(Regex("""[^а-яa-zё]+""")).filter { it != "" }
                .forEach { map[it] = map.getOrDefault(it, 0) + 1 }
    }
    val list = map.toList().sortedByDescending { it.second }
    return if (list.size < 20) list.toMap()
    else list.subList(0, 20).toMap()
}
...
fun transliterate(inputName: String, dictionary: Map<Char, String>, outputName: String) {
    ...
    File(outputName).writeText(File(inputName).readText().map {
        if (it.isLowerCase() || !it.toString().contains(Regex("""[A-Za-zА-я]""")))
...
fun chooseLongestChaoticWord(inputName: String, outputName: String) {
    ...
    if (lineList.all { it.isBlank() }) File(outputName).writeText("")
    else {
