 ...
 */
fun diameter(vararg points: Point): Segment = try {
    var maxDistance = -1.0
    var result = Segment(points[0], points[1])
    for (i in 0 until points.size - 1)
        for (c in i + 1 until points.size) {
            val distance = points[i].distance(points[c])
            if (distance > maxDistance) {
                result = Segment(points[i], points[c])
...
fun lineBySegment(s: Segment): Line {
    val angle = atan2(abs(s.end.y - s.begin.y), abs(s.end.x - s.end.y))
